...
class Sales extends Secure_area
{
	...
	{		
		$data=array();
		$this->form_validation->set_rules('amount_tendered', 'lang:sales_amount_tendered' );  // , 'numeric'   alterado!
		...
		if ( $payment_type == $this->lang->line('sales_giftcard') )  // Se usar o giftcard:
		{
		...
		else // adiciona o pagamento normal.
		{
			// Original: $payment_amount=$this->input->post('amount_tendered');

			// alterado:
			$payment_amount= to_currency_ingles( $this->input->post('amount_tendered') );  // adiciona o pagamento normal.
		}
		
		if( !$this->sale_lib->add_payment( $payment_type, $payment_amount ) )
		{
			$data['error']='Não é possível adicionar o pagamento, tente de outra forma!';
		}
	...
	function edit_item($line)
	{
		...
		$this->form_validation->set_rules('price', 'lang:items_price', 'required');  // alterado: |numeric
		...
		// Original:  $price = $this->input->post("price");   
		$price = to_currency_ingles( $this->input->post("price") );   // alterado!
