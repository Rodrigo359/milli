...
<div id="receipt_wrapper">
	...
	<td colspan="5" style='text-align:right;border-top:2px solid #000000;'><?php echo to_currency($subtotal); ?></td>
	</tr>

	<?php foreach($taxes as $name=>$value) { ?>
		<tr>
			<td colspan="5" style='text-align:right;'><?php echo $name; ?>:</td>
			<td colspan="5" style='text-align:right;'><?php echo to_currency($value); ?></td>
		</tr>
	<?php }; ?>

	<tr>
	<td colspan="5" style='text-align:right;'><?php echo $this->lang->line('sales_total'); ?></td>
	<td colspan="5" style='text-align:right'><?php echo to_currency($total); ?></td>
	</tr>
	...
	<tr>
		...
		<?php 
			echo "Formas de Pagamento: "; // antigo: $this->lang->line('sales_payment')
		?>
		</td>
	</tr>
	...
	?>
		...
		<td colspan="5" style="text-align:right;">
			<?php 
				$splitpayment=explode(':',$payment['payment_type']); echo $splitpayment[0];   // formas de pagamento.
			?> 
		</td>
		<td colspan="5" style="text-align:right">
			<?php 
				echo to_currency( $payment['payment_amount']  ); // antigo: * -1 , atualmente está mostrando o valor positivo.
			?> 
		</td>
	    </tr>
	<?php
    ...
    <?php 
    	$valorTroco = SUBSTR($amount_change , 2 , 3);     	// converter para numeral para comprar se é maior que zero(0).
    ?>

	<tr>
		<td colspan="5" style='text-align:right;'>
			<?php 
				 // echo $this->lang->line('sales_change_due');  // Valor devido (antigo).
				if ($valorTroco > 0)  // Só mostrar quando for troco:
				{
					echo "Troco: ";
				}
			?>
	...
	<!--   Alterado: removido o código de barrass da impressão
	<div id='barcode'>
	<? //php echo "<img src='index.php/barcode?barcode=$sale_id&text=$sale_id&width=250&height=50' />"; ?>
	</div> 
	-->
</div>
<?php // $this->load->view("partial/footer");  // Alterado: removido os dados do contato de propóito. ?>
